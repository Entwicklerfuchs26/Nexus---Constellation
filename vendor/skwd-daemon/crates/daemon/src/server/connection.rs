...
pub async fn run() -> anyhow::Result<()> {
    ...
    {
        // ManagedProcess only reaps its child via try_wait() when is_running() is called
        // before the next launch/toggle/kill on that same process. If a managed process
        // (wall-ui, host, music) exits on its own between such calls, it zombies until
        // someone touches it again. Poll all three periodically so crashes get reaped
        // promptly instead of accumulating as <defunct> children of the daemon.
        let state_for_reap = state.clone();
        tokio::spawn(async move {
            loop {
                tokio::time::sleep(std::time::Duration::from_secs(5)).await;
                state_for_reap.ui.lock().await.is_running();
                state_for_reap.host.lock().await.is_running();
                state_for_reap.music_proc.lock().await.is_running();
            }
        });
    }
        ...
        match &evt {
            ...
            watcher::FsEvent::WeRemoved { we_id } => {
                ...
                }
                {
                    let config = state.config.read().await.clone();
                    let cache_dir = config.cache_dir().join("wallpaper");
                    let _ = std::fs::remove_file(cache_dir.join("thumbs").join(format!("{we_id}.webp")));
                    let _ = std::fs::remove_file(cache_dir.join("thumbs-sm").join(format!("we-{we_id}.webp")));
                }
